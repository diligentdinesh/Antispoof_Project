...
class ASVspoof2019(tf.keras.utils.Sequence):
    def __init__(self, access_type, path_to_features, path_to_protocol, part='train', feature='LFCC',
        ...
        protocol = os.path.join(self.path_to_protocol, f'ASVspoof2019.{access_type}.cm.{self.part}.trl.txt')
    ...
    def __getitem__(self, idx):
        ...
        try:
            with open(os.path.join(self.ptf + "/" + filename + f'{self.feature}.pkl'), 'rb') as feature_handle:
            ...
            def the_other(train_or_dev):
                ...
                return res
            with open(os.path.join(self.path_to_features, the_other(self.part) + '/' + filename + f'{self.feature}.pkl'), 'rb') as feature_handle:
                feat_mat = pickle.load(feature_handle)
