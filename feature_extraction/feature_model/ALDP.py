...
def aldp_threshold_neg(x, threshold):
    if x >= threshold:
        ...
        return 0
def calculate_alpha_parameter(frame, center_value):
    frame = np.array(frame)
...
def ALDP(sound_signal, frame_length=9):
    ...
    ALDP = np.concatenate((ALDP_pos, ALDP_neg))
    return ALDP
